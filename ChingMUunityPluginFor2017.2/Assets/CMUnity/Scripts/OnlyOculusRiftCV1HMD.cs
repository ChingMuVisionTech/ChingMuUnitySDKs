...
另外如果没有sensor，但是头显自带的旋转依然存在，这个旋转会实时应用到子物体 VR Camera上，当头显的正方向与青瞳追踪的正方向不一致时，应该抵消这个旋转，通过API CMUnity.CMQuatWithImu()，可以纠正头显的正方向与青瞳追踪正方向之间的偏差，所以头显自带的旋转可以用这个API处理；
*/
